...
| **`RST &08` codes** | A DOS error name from [errors.md](../ref/masterdos/docs/errors.md), or a hook code — 128 plus an index into the image's own hook table at `&44A6`, so the code is named after the routine it dispatches to (`HOOK_HOFLE`, `HOOK_MCHRD`). |
