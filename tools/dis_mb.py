...
TITLES = {
    'DOS': '\n'.join((
        ...
        '; Generated by tools/dis_mb.py.  It is assembled through',
        '; base.asm, which INCLUDEs both halves and holds the equates',
        '; they share; that reproduces this half byte for byte.',
        ';',
...
; meaning &8009 after BOOT had moved.  Written as BOOT + %s it is a
...
def write_trio(outdir, dos, mb, texts, bias, preamble=None):
    ...
    `texts` is each half as it would have been written on its own.  What
    changes here is that the equates both halves declare are lifted out
    ...
    if clash:
        for note in clash:
            print('base.asm: the halves disagree -- ' + note)
    ...
    for d, other, label in ((dos, mb, 'MasterDOS reaching MasterBASIC'),
                            (mb, dos, 'MasterBASIC reaching MasterDOS')):
        lines, bad = basefile.peer_equates(d, other, bias)
        if lines:
            peer_lines.append('')
            peer_lines.append('; ' + label + '.')
            peer_lines.extend(lines)
        unresolved += bad
    ...
    body.append('')
    body.append('%-14s ORG  %s' % ('', hexn(BASE, 4)))
    body.append('%-14s DUMP 1,&0000' % '')
    body.append('%-14s INC  "masterbasic.asm"' % '')
    body.append('')
    body.append('; The two halves, each reaching the other.  These come after')
    body.append('; the INCLUDEs because they name labels the INCLUDEs define;')
    body.append("; pyz80's second pass is what makes that legal.")
    body.extend(peer_lines)
    kept, dropped = basefile.prune(
        chr(10).join(body).split(chr(10)),
        [texts['masterdos.asm'], texts['masterbasic.asm']])
    if dropped:
...
def write_clean(pages):
    ...
    for d, name in ((dos, 'masterdos.asm'), (mb, 'masterbasic.asm')):
        ...
        texts[name] = body
    write_trio(out, dos, mb, texts, PAGE_BIAS[0],
               preamble=clean.base_preamble())
    ...
    if args.outdir:
        ...
        for d, name in ((dos, 'masterdos.asm'), (mb, 'masterbasic.asm')):
            ...
            if gone:
                print('%s: dropped %d equates nothing refers to' % (name, gone))
            texts[name] = body
        write_trio(args.outdir, dos, mb, texts, '&4000')
