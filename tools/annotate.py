...
MB = {
    ...
    **dict((a, (hooks.NAMES[a], hooks.DOCS[a])) for a in hooks.NAMES),
    ...
    # Both halves carry these; MasterDOS keeps the bare names, being
    # where they were first written down, and these take an MB prefix
    # so that base.asm can assemble the two together.  See
    # nrfam.SHARED_NAMES, which is the list both ends work from.
    0x44F0: ('MBCMR', CMR_DOC),
    0x455F: ('MBNRRDD', NR_DOC),
    0x456A: ('MBNRRD', None),
    0x4577: ('MBNRWRD', None),
    0x4582: ('MBNRWR', None),
    0x45A4: ('WRA', None),
