...
EDITOR_RETURN:        EQU  &5A62               ; REF: the ROM's return into its main loop, less three, parked across the
                                               ; editor call
EDITV:                EQU  &5AEC               ; vector taken by the editor
EDIT_PENDING:         EQU  &5A60               ; EDIT: &FF from hook 183 before the ROM's INPUT runs; the routine hook
                                               ; 185 plants reads and clears it
...
REF_CURSOR:           EQU  &5A65               ; REF: KCUR less one, caught by the editor hook the last time it was
                                               ; still short of the match
REF_MATCH_END:        EQU  &5A5E               ; REF: the address just past the match, for the editor hook to catch the
                                               ; cursor against
...
SAVED_CHANNEL_OUTPUT: EQU  &5A67               ; REF: the ROM's own channel output word, put aside while MasterBASIC's
