...
; ---- BOOT_8 ---- from &4081 when B is not 0 yet
BOOT_8:
               DJNZ BOOT_8                     ; 4081 10 FE
...
BOOT_LOOP5:
               ...
               AND &0D                         ; 409D E6 0D
               JR Z,BOOT_11                    ; 409F 28 1F
               LD A,(V40FD+&4000)              ; 40A1 3A FD 80
               INC A                           ; 40A4 3C
               LD (V40FD+&4000),A              ; 40A5 32 FD 80
               PUSH AF                         ; 40A8 F5
               AND &02                         ; 40A9 E6 02
               JR Z,BOOT_10                    ; 40AB 28 08
               LD A,&09                        ; 40AD 3E 09
               OUT (C),A                       ; 40AF ED 79
               LD B,&14                        ; 40B1 06 14

; ---- BOOT_9 ---- from &40B3 when B is not 0 yet
BOOT_9:
               DJNZ BOOT_9                     ; 40B3 10 FE

; ---- BOOT_10 ---- from &40AB when no bit of &02 is set
BOOT_10:
               POP AF                          ; 40B5 F1
               CP &0A                          ; 40B6 FE 0A
...
;;        status and data registers. MasterDOS instead patches the port numbers into the IN and OUT instructions before
;;        the
...
;;
;;        A dirty flag. Bit 3 of (IX+MFLG) records that the sector in the buffer has been written to, so WRIF can flush
;;        it
...
OFSM_1:
               ...
               AND A                           ; 4D51 A7
               JR Z,OFSM_3                     ; 4D52 28 1A  NO "OVERWRITE? Y/N" IF SAVE OVER
               PUSH HL                         ; 4D54 E5
               ...
               JR Z,OFSM_2                     ; 4D64 28 05  JR IF "Y"
               POP DE                          ; 4D66 D1
               POP IX                          ; 4D67 DD E1
...
DBAML:
               ...
               JR Z,OFSM_1                     ; 4D94 28 A5  JR IF SECOND OR OTHER COPY...
...
;;
;; Nothing here is about a channel being open, which the name this
;; carried for a while claimed.
;; --------------------------------------------------------------------
...
; ---- DLVM1 ---- from &5FC0 when A <> &14, &5FC5
DLVM1:
               CP &10                          ; 5FF7 FE 10
...
RDCODE:
               ...
               LD SP,MB_FN_NVAL_POSITIVE       ; 77CD 31 00 82
...
;; --------------------------------------------------------------------
;; Of the 26 instructions between this label and the next routine the source documents, 17 survive into this image, so
