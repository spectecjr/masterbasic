...
ROM_CHKHL:     EQU  &3FEF  ; Checks if HL is in the range C000-FFFF, and if so, adjusts it back into the range 8000-BFFF, and increments the upper page.
...
RPTH:          EQU  &0E    ; and high -- 0 or 1, selecting one of the two entries in a directory sector
...
L4018:
               LD A,(HL)                       ; 4018 7E
               CP &D0                          ; 4019 FE D0
               JR C,L4023                      ; 401B 38 06
               CP &D8                          ; 401D FE D8
               JR NC,L4023                     ; 401F 30 02
               LD (HL),&00                     ; 4021 36 00
...
; ---- L4023 ---- from &401B, &401F
L4023:
               DEC L                           ; 4023 2D
               JR NZ,L4018                     ; 4024 20 F2
...
L4033:
               LD A,(HL)                       ; 4033 7E
               AND A                           ; 4034 A7
               JR Z,L4040                      ; 4035 28 09
               CP &30                          ; 4037 FE 30
               JR Z,L4040                      ; 4039 28 05
               DEC L                           ; 403B 2D
               JR NZ,L4033                     ; 403C 20 F5
...
;; Leaves:    F, C
;;
;; ? drives OUT (C),E; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
; ---- L4061 ---- from &4064, &4079, &40B8
L4061:
               IN A,(C)                        ; 4061 ED 78
               RRA                             ; 4063 1F
               JR C,L4061                      ; 4064 38 FB
               INC C                           ; 4066 0C
               IN A,(C)                        ; 4067 ED 78

; ---- L4069 ---- from MB &6099, MB &60CA
L4069:
               DEC C                           ; 4069 0D
               CP D                            ; 406A BA

; ---- L406B ---- from MB &6095, MB &60CE
L406B:
...
;; Leaves:    A, F, B
;;
;; ? drives IN A,(C), OUT (C),A; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------

; ---- L4094 ---- from &408A, &409B
L4094:
               IN A,(C)                        ; 4094 ED 78
               BIT 1,A                         ; 4096 CB 4F
               JR NZ,L408C                     ; 4098 20 F2
               RRCA                            ; 409A 0F
               JR C,L4094                      ; 409B 38 F7
               AND &0D                         ; 409D E6 0D
               JR Z,L40C0                      ; 409F 28 1F
               LD A,(V40FD+&4000)              ; 40A1 3A FD 80
               INC A                           ; 40A4 3C
               LD (V40FD+&4000),A              ; 40A5 32 FD 80
               PUSH AF                         ; 40A8 F5
               AND &02                         ; 40A9 E6 02
               JR Z,L40B5                      ; 40AB 28 08
               LD A,&09                        ; 40AD 3E 09
               OUT (C),A                       ; 40AF ED 79
               LD B,&14                        ; 40B1 06 14
...
L40E0:
               ...
               AND &1F                         ; 40ED E6 1F
               DEC A                           ; 40EF 3D
               ; self-modifying: patches the operand of the LD at &42CC
...
ENTSP:
               DEFB &00,&00                                                     ; 4104 ..  SP on entry to a command; SETSTK saves the previous value behind it

; ---- SNPRT0 ---- from &53C1, &5475, &5FD0
SNPRT0:
               DEFB &1F                                                         ; 4106 .  LMPR, HMPR and VMPR of the interrupted machine, saved by the NMI code
...
CCHAD:
               DEFB &00,&00                                                     ; 410C ..  CHADD saved on entry, so it can be put back for an external command
...
DSC:
               DEFB &00                                                         ; 4110 .  base port of the WD1772, with drive and side selection in the high bits
...
; ---- DST ---- from &46F4, &4701
DST:
               DEFB &00,&00,&00,&00,&00,&00,&00,&00,&00,&00,&00                 ; 4112 ...........  track read back by "read address" when confirming the head position

; ---- SVA ---- from &4FF9, &5003, &56FB, &5715
SVA:
               DEFB &00                                                         ; 411D .  one-byte scratch, to carry a value across a call that corrupts A
...
CHDIR:
               ...
               DEFB &01                                                         ; 4226 .  6
               DEFB &32                                                         ; 4227 2  skipped: reads as LD (&0100),A from here, and as part of the instruction above it
...
ST3HP:
               ...
               ; read the ROM variable XPTR -- the word below is its address, and the call returns past it
               CALL NRRDD                      ; 437C CD 53 50
               DEFW XPTR                      ; 437F A3 5A
               LD (V42E4),BC                   ; 4381 ED 43 E4 42
               ; read the ROM variable CSTAT -- the word below is its address, and the call returns past it
...
L43B5:
               ; call &0000 in the other page: LMPR is switched first, so that address is how the other listing numbers it
...
CKESV:
               ...
               JR Z,L43E5                      ; 43D2 28 11
               BIT 7,H                         ; 43D4 CB 7C
               JP Z,L423E                      ; 43D6 CA 3E 42
...
;; Leaves:    A, F, BC, HL
;;
;; ? reaches the ROM through BSTKEND; calls NRRDD; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------

; ---- L43E5 ---- from &4360, &43D2
L43E5:
               CP &04                          ; 43E5 FE 04
               JR Z,L440B                      ; 43E7 28 22
               CP &1A                          ; 43E9 FE 1A
               JR NZ,L4425                     ; 43EB 20 38
               ; read the ROM variable BSTKEND -- the word below is its address, and the call returns past it
               CALL NRRDD                      ; 43ED CD 53 50
...
;; Leaves:    A, F, BC, HL
;;
;; ? reaches the ROM through BSTKEND; calls NRWRD, RDBC; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------

; ---- L43F4 ---- from &43F9
L43F4:
               CALL RDBC                       ; 43F4 CD BA 50
               LD A,B                          ; 43F7 78
               OR C                            ; 43F8 B1
               JR NZ,L43F4                     ; 43F9 20 F9
               INC HL                          ; 43FB 23
               RES 7,H                         ; 43FC CB BC
               SET 6,H                         ; 43FE CB F4
               LD B,H                          ; 4400 44
...
SAMHK:
               ...
               DEFW MB_HK_SUBCHAR+NOT_IN_THIS_PAGE ; 4512 code 182
...
;; --------------------------------------------------------------------
;; READ_SELECTED_DISK_STATUS -- &4526 to &452D
;;
;; Takes:     A, BC
;; Leaves:    A, F
...
;; Ends:      RET
;;
;; ? drives IN A,(C); calls GET_DISK_PORT_BASE.
;; --------------------------------------------------------------------
...
;; --------------------------------------------------------------------
;; WRITE_SELECTED_DISK_CMD -- &452E to &4535
;;
;; Takes:     A, BC
;; Leaves:    registers unchanged
;; Preserves: BC (saved and restored)
;; Ends:      RET
;;
;; ? drives OUT (C),A; calls GET_DISK_PORT_BASE.
;; --------------------------------------------------------------------
...
;;     
;;     On entry, DE is about to be decremented. The routine checks if DE would cross
;;     a page boundary as it is decremented.
;;     
;;     If it would, the PGES1 value is decremented, and the DE value is adjusted upwards by 16K.
;; --------------------------------------------------------------------
...
;; --------------------------------------------------------------------
;; WAIT_DC_READY_BEFORE_CMD -- &4558 to &455A
;;
;; Takes:     A, BC
;; Leaves:    A, F
;;
...
; ---- WAIT_DC_READY_BEFORE_CMD ---- from &4554, &456A, &46F1, &478D, &48B4, &4F9D, &54B7
WAIT_DC_READY_BEFORE_CMD:
               CALL BUSY                       ; 4558 CD 6D 45

;; --------------------------------------------------------------------
;; WRITE_DRIVE_CMD_AND_DELAY -- &455B to &4560
;;
;; Takes:     BC
;; Leaves:    A, B
;;
;; ? calls WRITE_SELECTED_DISK_CMD; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
; ---- BUSY ---- from &4558, &4576, &474E, &47BF, &7A01
BUSY:
...
WRIF:
               CALL L4FBF                      ; 4578 CD BF 4F  GET DE=CUR SECTOR
...
;; Leaves:    A, F, C
;;
;; ? calls DWAIT; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
WSA1:
               ...
               ; self-modifying: patches the operand of the IN at &45AE
...
;; --------------------------------------------------------------------
;; CHECK_WRITE_STATUS -- &45AE to &45B6
;;
;; Takes:     nothing in registers
;; Leaves:    A, F
;; Ends:      JR
...
CHECK_WRITE_STATUS:
               IN A,(&00)                      ; 45AE DB 00  Read the drive controller status port (specific port chosen by code above)
               RRCA                            ; 45B0 0F
...
;; --------------------------------------------------------------------
;; CHECK_READ_STATUS -- &45D9 to &45E1
;;
;; Takes:     HL
;; Leaves:    A, F, HL
;; Ends:      JR
...
; ---- CHECK_READ_STATUS ---- from &45D3, &45DE
CHECK_READ_STATUS:
               IN A,(&00)                      ; 45D9 DB 00  Read the drive controller status port (specific port chosen by code above)
               RRCA                            ; 45DB 0F
...
L45E2:
               ...
               CP &04                          ; 45EE FE 04
               LD A,E                          ; 45F0 7B
               JR NZ,L45F9                     ; 45F1 20 06
               CP &02                          ; 45F3 FE 02
               JR NZ,L4612                     ; 45F5 20 1B
               DEC A                           ; 45F7 3D
               LD E,A                          ; 45F8 5F
...
L45FF:
               ...
               JR NZ,L45FF                     ; 460A 20 F3
               LD HL,DRAM                      ; 460C 21 13 7D
...
NRDDATA:
               ...
               ; self-modifying: patches the operand of the IN at &4666
               LD (TGT1+1),A                   ; 4651 32 67 46  patches the port of the IN at &4666
               ; self-modifying: patches the operand of the IN at &4676
               LD (NRSA3+1),A                  ; 4654 32 77 46  patches the port of the IN at &4676
               ADD A,DISKCTL_DATA_OFS          ; 4657 C6 03
               ; self-modifying: patches the operand of the IN at &466B
...
SRSA1:
               ...
               ; self-modifying: patches the operand of the IN at &46B3
               LD (SRSA3+1),A                  ; 468E 32 B4 46  patches the port of the IN at &46B3
               ADD A,DISKCTL_DATA_OFS          ; 4691 C6 03
               ; self-modifying: patches the operand of the IN at &46AC
...
; ---- SRSA3 ---- from &469F, &46B8, &46BF, &46C4
SRSA3:
...
;; Ends:      JR, RET
;;
;; ? drives OUT (C),A; calls TRCKP, WAIT_DC_READY_BEFORE_CMD, RDDATA, REST.
;; --------------------------------------------------------------------
...
;; Leaves:    A, F, BC, DE, HL
;;
;; ? calls BCC; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
;; Leaves:    A, F, BC
;;
;; ? drives OUT (C),E; calls SELD; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------

; ---- SEEKD ---- from &4733, &4736
SEEKD:
...
;; Leaves:    A, F, BC, HL
;;
;; ? drives IN A,(C); calls TRCKP, BUSY, BITF6; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
;;
;; ? calls STEP; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
;; Leaves:    A, F
;;
;; ? calls WAIT_DC_READY_BEFORE_CMD; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
;; Leaves:    A, F, BC, DE
;;
;; ? calls TFIHO, TIRD; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
; ---- REP10 ---- from &4805, &6339, &6353, &7032
REP10:
...
;; Ends:      JR
;;
;; ? calls CCNT, TIRD, RDLB.
;; --------------------------------------------------------------------

; ---- LDB3 ---- from &48A6, &48FC
LDB3:
               CALL CCNT                       ; 4894 CD 0C 49
               JP C,L48FF                      ; 4897 DA FF 48  less than a whole sector left: finish byte by byte
               INC HL                          ; 489A 23
               LD (SVDE),HL                    ; 489B 22 02 7C
               CALL TIRD                       ; 489E CD 5A 61
               JR C,LDB35                      ; 48A1 38 05
...
;; Ends:      JR
;;
;; ? drives IN A,(&00); calls WAIT_DC_READY_BEFORE_CMD, CDE1, CTAS, GTBUF.
;; --------------------------------------------------------------------
...
LDB4:
               ...
               ; self-modifying: patches the operand of the IN at &48DB
               LD (LDB6+1),A                   ; 48C1 32 DC 48  STATUS PORT
               ADD A,DISKCTL_DATA_OFS          ; 48C4 C6 03
               ; self-modifying: patches the operand of the IN at &48D2
...
HSVB2:
               ...
               CALL L4FC6                      ; 494D CD C6 4F  SAVE DE TO NEXT T/S VARIABLES
...
;; Leaves:    A, F, D, HL
;;
;; ? calls CKDE, SETF6; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
Fix_L4861_4x:
               ...
               LD (HL),E                       ; 498C 73  *
               EX DE,HL                        ; 498D EB  *
               CALL L4FCD                      ; 498E CD CD 4F  L6CAB / L6CAE / L6CA2_SWPNSR ?;*
...
;; Leaves:    A, F, BC, DE, HL
;;
;; ? calls CCNT, FFNS; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
SVB3:
               ...
               CALL L4FBF                      ; 49DB CD BF 4F  GET T/S IN D/E
               ; to the alternate register set and back again
               EXX                             ; 49DE D9
...
;; Leaves:    A, F, BC, DE, HL
;;
;; ? calls PRECMX, CTAS; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
SVBF:
               ...
               ; self-modifying: patches the operand of the IN at &4A28
...
;; Ends:      JP, JR
;;
;; ? drives IN A,(&00), OUT (HMPR),A; calls PRECMX, CTAS.
;; --------------------------------------------------------------------

; ---- SVB45 ---- from &4A13, &4A1B
SVB45:
...
;; Ends:      JP, JR
;;
;; ? drives IN A,(&00), OUT (HMPR),A; calls PRECMX, CDE1, CTAS.
;; --------------------------------------------------------------------
...
SVB7:
               ...
               ; to the alternate register set and back again
               EXX                             ; 4A57 D9
               JP SVB4                         ; 4A58 C3 E3 49  TRY AGAIN - MAX 10 ERRORS IN
...
; ---- SVBS3 ---- from &4A6B
SVBS3:
               CALL L4FBF                      ; 4A73 CD BF 4F  GET T/S
...
; ---- FFNS ---- from &49B0, &7614
FFNS:
               LD DE,(L42E8)                   ; 4A78 ED 5B E8 42
               PUSH HL                         ; 4A7C E5
               PUSH BC                         ; 4A7D C5
               LD HL,(L42E6)                   ; 4A7E 2A E6 42
               JR FNS2                         ; 4A81 18 09
...
FNS3:
               ...
               LD (L42E8),DE                   ; 4AA1 ED 53 E8 42  remember where to resume
               LD (L42E6),HL                   ; 4AA5 22 E6 42
...
;; Leaves:    A, F, B, DE, HL
;;
;; ? calls CKNAM; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
;; Leaves:    A, F, BC, DE, HL, IY
;;
;; ? calls PNUM2, SPC; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------

; ---- FDH4A ---- from &4C03
FDH4A:
               CALL L4FD7                      ; 4C12 CD D7 4F  GET FILE NUMBER IN BC
               PUSH DE                         ; 4C15 D5
               ...
               LD L,C                          ; 4C1D 69
               LD A,&20                        ; 4C1E 3E 20
               JR C,L4C29                      ; 4C20 38 07  JR IF >99
               CALL PNUM2                      ; 4C22 CD 35 57
...
;; Leaves:    BC, DE
;;
;; ? calls PNUM3; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------

; ---- L4C29 ---- from &4C20
L4C29:
               CALL C,PNUM3                    ; 4C29 DC 2F 57
               POP DE                          ; 4C2C D1
...
;; Leaves:    A, F, BC, DE, HL
;;
;; ? calls PFNME, POINT, PNUM3, SPC; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
;; Leaves:    A, F, BC, DE
;;
;; ? calls PNUM4; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------

; ---- FDH84 ---- from &4C4A
FDH84:
               CALL C,PNUM4                    ; 4C53 DC 29 57  USE 4 DIGITS, BUTTING ONTO TYPE,
               POP AF                          ; 4C56 F1
...
CKNAM:
               ...
               JR Z,L4CDF                      ; 4CCF 28 0E  JR IF TYPE IRREL - SKIP TYPE
...
CKNM1:
               LD A,(DE)                       ; 4CD1 1A
               CP &2A                          ; 4CD2 FE 2A
               JR Z,L4CE6                      ; 4CD4 28 10  MATCH ANY LEN IF "*"
               CP &3F                          ; 4CD6 FE 3F
               JR Z,L4CDF                      ; 4CD8 28 05  MATCH ANY CHAR IF "?"
               XOR (HL)                        ; 4CDA AE
               AND &DF                         ; 4CDB E6 DF
               JR NZ,L4CE4                     ; 4CDD 20 05
...
L4CE6:
               DEC B                           ; 4CE6 05
               JR Z,L4CE3                      ; 4CE7 28 FA
               INC DE                          ; 4CE9 13
               INC HL                          ; 4CEA 23
               LD A,(DE)                       ; 4CEB 1A
               CP &2E                          ; 4CEC FE 2E
               JR NZ,L4CE3                     ; 4CEE 20 F3  E.G. "*" OR "A*" MATCHES ANYTHING
...
L4CF0:
               LD A,(HL)                       ; 4CF0 7E
               CP &2E                          ; 4CF1 FE 2E
               JR Z,L4CDF                      ; 4CF3 28 EA  IF MATCHING "." FOUND,
               INC HL                          ; 4CF5 23
               DJNZ L4CF0                      ; 4CF6 10 F8
               POP DE                          ; 4CF8 D1
...
ROFSM:
               ...
               DEC A                           ; 4D0F 3D
               JR NZ,L4D22                     ; 4D10 20 10  JR IF NOT FIRST FILE
...
;; Leaves:    A, F, BC, DE, HL, IX, IY
;;
;; ? calls FDHR, CLSAM; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
;; Leaves:    A, F, BC, DE, HL
;;
;; ? calls POINT, DDEL; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
;;
;; ? calls WSAD, FDH1; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
DBAML:
               ...
               JR Z,L4D3B                      ; 4D94 28 A5  JR IF SECOND OR OTHER COPY...
...
;; Leaves:    A, F, BC, DE, HL, IY
;;
;; ? reaches the ROM through BEEPR; calls CMR; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
;; Leaves:    A, F, BC, DE, HL, IX, IY
;;
;; ? calls FDHR; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------

; ---- CLOSX ---- from &4E06, &6ED5
CLOSX:
...
NCF25:
               ...
               JR NZ,L4E43                     ; 4E2C 20 15  JR IF NOT T0,S1,ENTRY1
...
;; Leaves:    A, F, B, HL, IX
;;
;; ? reaches the ROM through CURCMD; calls CFMC, NRRD; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
L4E43:
               ...
               ; read the ROM variable CURCMD -- the word below is its address, and the call returns past it
               CALL NRRD                       ; 4E4A CD 5E 50  COPY DATE/TIME TO +F2 TO +FB
               DEFW CURCMD                    ; 4E4D 74 5B
               CP &CF                          ; 4E4F FE CF
               JR Z,L4E58                      ; 4E51 28 05
               ; call &4A39 in the other page: LMPR is switched first, so that address is how the other listing numbers it
...
;; Ends:      JR, RET
;;
;; ? calls NMMOV, LCNTA, POINT, GRPNTB.
;; --------------------------------------------------------------------

; ---- L4E75 ---- from &5FA7, &65F7
L4E75:
               CALL L4F6C                      ; 4E75 CD 6C 4F  FIND FILE
               AND &1F                         ; 4E78 E6 1F
               CP &15                          ; 4E7A FE 15
...
;; Leaves:    A, F, HL
;;
;; ? calls BITF7; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
;; Ends:      JR, RET
;;
;; ? calls NMMOV, LCNTA, GRPNTB, PAGEFORM.
;; --------------------------------------------------------------------
...
; ---- L4F67 ---- from &4E91, &4EB9, &4F11, &4F69
L4F67:
               LD (DE),A                       ; 4F67 12
               INC DE                          ; 4F68 13
               DJNZ L4F67                      ; 4F69 10 FC
               RET                             ; 4F6B C9
...
;; Leaves:    A, F, HL
;;
;; ? calls SETF7; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------

; ---- GTFS1 ---- from &4F74
GTFS1:
               CP &10                          ; 4F78 FE 10
               JR NC,L4F82                     ; 4F7A 30 06  JR IF SAM FILE
               DEC A                           ; 4F7C 3D
               OR &10                          ; 4F7D F6 10
...
; ---- L4F84 ---- from &4370, &4B33, &4D28, &557D, &5E7E, &5F7A, &6428, &647F ...
L4F84:
...
; ---- L4F8E ---- from &46CA, &4C92, &4DD2, &5D65, &5EBD, &74E4, &7782
L4F8E:
...
;; Leaves:    A, F, C
;;
;; ? calls WAIT_DC_READY_BEFORE_CMD, CTAS; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
; ---- L4FBF ---- from &4578, &48E8, &49DB, &4A73, &4DF8, &4FCD, &70F3
L4FBF:
...
; ---- L4FC6 ---- from &48AC, &494D, &4DC9, &6FD9
L4FC6:
...
; ---- L4FCD ---- from &498E, &6F85, &6FA4, &6FC5, &761D
L4FCD:
               CALL L4FBF                      ; 4FCD CD BF 4F
...
; ---- L4FD7 ---- from &4C12, &7B46
L4FD7:
               LD A,(IX+&0E)                   ; 4FD7 DD 7E 0E
               ; call &4224 in the other page: LMPR is switched first, so that address is how the other listing numbers it
...
;; Leaves:    A, F, HL
;;
;; ? calls CFSO; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
;; Leaves:    A, F, DE, HL
;;
;; ? reaches the ROM through XPTR+1; calls NRWR; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
; ---- GTNC ---- from &4394, &5035, &5997, &59BB, &59CC, &5AFA, &5B13, &5B45 ...
GTNC:
               ; call the ROM at NEXTCHAR with ROM1 paged in, and page back on the way out
...
GCHR:
               ; call the ROM at GETCHAR with ROM1 paged in, and page back on the way out
               CALL CMR                        ; 5042 CD B2 7B
               DEFW GETCHAR                   ; 5045 18 00
               RET                             ; 5047 C9

;; --------------------------------------------------------------------
;; ALPHANUM -- &5048 to &504B
;;
;; Takes:     A
;; Leaves:    F
;;
;; ? calls ALPHA; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
NUMERIC:
               CP &3A                          ; 504C FE 3A  NC IF TOO HIGH
               RET NC                          ; 504E D0
               CP &30                          ; 504F FE 30
               CCF                             ; 5051 3F
               RET                             ; 5052 C9
...
NRRDD:
               ; the return address becomes HL, so what follows the call is data this routine reads, not code
               EX (SP),HL                      ; 5053 E3
               PUSH DE                         ; 5054 D5
...
PLNS:
               ...
               ; read the ROM variable ERRSP -- the word below is its address, and the call returns past it
...
; ---- PHLR ---- from &50E1
PHLR:
               DEFB &E1,&C9                                                     ; 50EA aI  skipped: reads as POP HL from here, and as part of the instruction above it
...
SETBORDER_BORDCR:
               PUSH AF                         ; 5152 F5
               PUSH BC                         ; 5153 C5
               ; read the ROM variable ROM_BORDCR -- the word below is its address, and the call returns past it
...
; ---- REP13 ---- from &4E7C, &4E83, &4EDD, &5FFE
REP13:
               LD A,&5E                        ; 5171 3E 5E
               DEFB &21                                                         ; 5173 !  skipped: reads as LD HL,&633E from here, and as part of the instruction above it
...
REP18:
               LD A,&63                        ; 5174 3E 63
               DEFB &21                                                         ; 5176 !  skipped: reads as LD HL,&643E from here, and as part of the instruction above it
...
REP20:
               LD A,&65                        ; 517A 3E 65
               DEFB &21                                                         ; 517C !  skipped: reads as LD HL,&673E from here, and as part of the instruction above it
...
; ---- REP22 ---- from &4815, &4820, &7582, &7645, &7734
REP22:
               LD A,&67                        ; 517D 3E 67
               DEFB &21                                                         ; 517F !  skipped: reads as LD HL,&683E from here, and as part of the instruction above it
...
; ---- REP25 ---- from &4E18, &721A
REP25:
               LD A,&6A                        ; 5186 3E 6A
               DEFB &21                                                         ; 5188 !  skipped: reads as LD HL,&163E from here, and as part of the instruction above it
...
; ---- REP27 ---- from &473A, &6F01, &70BF, &71A5, &75CD, &7A5A
REP27:
               LD A,&16                        ; 5189 3E 16
               DEFB &21                                                         ; 518B !  EOF
...
REP31:
               LD A,&70                        ; 5192 3E 70
               DEFB &21                                                         ; 5194 !  skipped: reads as LD HL,PTRSL from here, and as part of the instruction above it
...
; ---- L519B ---- from &5A33, &6A11
L519B:
               LD A,&73                        ; 519B 3E 73
               DEFB &21                                                         ; 519D !  skipped: reads as LD HL,&743E from here, and as part of the instruction above it
...
; ---- L519E ---- from &4D5B, &5E5F
L519E:
               LD A,&74                        ; 519E 3E 74

;; --------------------------------------------------------------------
;; REPORTA -- &51A0 to &51A6
;;
;; Takes:     A, BC, DE, HL
;; Leaves:    A, F, HL
;;
...
;; Leaves:    A, F, BC, DE, HL, IY
;;
;; ? reaches the ROM through CHADD; calls NRRDD; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------

; ---- DERR1 ---- from &51B5
DERR1:
               LD A,D                          ; 51B9 7A
               CALL L51A7                      ; 51BA CD A7 51
               LD (PRTRK),A                    ; 51BD 32 17 52
               LD (FMTRK),A                    ; 51C0 32 32 52
               LD (PRTRK+1),BC                 ; 51C3 ED 43 18 52
               LD (FMTRK+1),BC                 ; 51C7 ED 43 33 52
               LD A,E                          ; 51CB 7B
               CALL L51A7                      ; 51CC CD A7 51
...
;; Leaves:    A, F, HL
;;
;; ? reaches the ROM through XPTR; calls NRWRD; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
L51E3:
               ...
               ; read the ROM variable CHADP -- the word below is its address, and the call returns past it
               CALL NRRD                       ; 51E8 CD 5E 50
               DEFW CHADP                     ; 51EB 96 5A
               ; write the ROM variable XPTRP
...
NMI:
               ...
               PUSH HL                         ; 536F E5
               LD (HKSP),SP                    ; 5370 ED 73 0B 42  an error during the save unwinds here, which resumes the program
...
;; Leaves:    F, BC, E
;;
;; ? drives IN E,(C); falls into whatever follows rather than returning.
;; --------------------------------------------------------------------

; ---- SNAP3 ---- from &53B6, &53F3
SNAP3:
...
;; Ends:      JP, JR
;;
;; ? drives IN A,(C); calls CKDRX, DSVBL, FDHR, OFSM.
;; --------------------------------------------------------------------
...
;; Leaves:    A, F, BC, DE, HL, IX, IY
;;
;; ? calls CKDRX, FDHR; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
;; Leaves:    A, F, BC, DE, HL, IX, IY
;;
;; ? calls OFSM, SVHD; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
; ---- SNAP8 ---- from &548F, &5493
SNAP8:
               ; the stack is being reset, so this path does not return
               LD SP,(STR)                     ; 5497 ED 7B 90 7F
               JP MB_L7900                     ; 549B C3 00 B9
...
DFMT:
               ...
               LD B,&0A                        ; 54A5 06 0A  step in a little, so the restore below has somewhere to come back from
...
DFMTA:
               ...
               INC A                           ; 54CB 3C
               CP &02                          ; 54CC FE 02
               JR C,L54D7                      ; 54CE 38 07
               LD A,(HL)                       ; 54D0 7E
               AND &7F                         ; 54D1 E6 7F
               CP &2A                          ; 54D3 FE 2A
               JR NZ,L54DC                     ; 54D5 20 05
...
;; Ends:      JR
;;
;; ? calls CALLMB, GETSCR, PMOA, PNDN2.
;; --------------------------------------------------------------------
...
;; Leaves:    HL
;;
;; ? calls PM7K; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
DFMTB:
               ...
               ; call &5352 in the other page: LMPR is switched first, so that address is how the other listing numbers it
...
FMT1A:
               ...
               DEC E                           ; 5528 1D  E=0-9
               ADD A,E                         ; 5529 83
               JR C,L552E                      ; 552A 38 02  JR IF E.G. FF+02=01
...
;; Leaves:    A, F, BC, DE
;;
;; ? calls REST; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
FMT7:
               ...
               CP &FF                          ; 5542 FE FF
               JR Z,L558F                      ; 5544 28 49  JR IF SIMPLE FORMAT, NOT DISK COPY
...
;; Leaves:    A, F, HL
;;
;; ? calls PMOC; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
;; Leaves:    A, F, DE, HL
;;
;; ? calls SCTRK, ITRCK; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
FESE2:
               ...
               LD (HL),A                       ; 55FD 77
               DEC HL                          ; 55FE 2B
               ; read the ROM variable FRAMES -- the word below is its address, and the call returns past it
...
;; Ends:      JP, JR
;;
;; ? drives IN A,(HMPR), OUT (HMPR),A; calls GRPNTB, PNUM5.
;; --------------------------------------------------------------------
...
PNTY5:
               ...
               CALL L56BB                      ; 56B5 CD BB 56  PRINT DATE/TIME
...
PNUM6:
               ...
               RR D                            ; 5704 CB 1A
               RR C                            ; 5706 CB 19
               RR D                            ; 5708 CB 1A  the low two bits of the page, aligned to bit 14 of the total
               LD A,D                          ; 570A 7A
               ADD A,H                         ; 570B 84
               LD H,A                          ; 570C 67
               LD A,C                          ; 570D 79
...
;; Leaves:    A, F, BC, DE
;;
;; ? calls PNM2; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
;; Leaves:    BC, DE
;;
;; ? calls PNM1; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
;; Leaves:    A, F, BC, DE, HL, IY
;;
;; ? calls CLSL; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
;; Leaves:    A, F, DE, HL
;;
;; ? reaches the ROM through INQUFG; calls NRWR; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
;; Leaves:    B
;;
;; ? calls PFNME; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
;; Leaves:    A, F, BC, DE, HL, IY
;;
;; ? calls RDKY; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
;; Leaves:    A, F, BC, DE, HL, IY
;;
;; ? calls RDKY, CLSL; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
;; Leaves:    A, F, BC, DE, HL, IY
;;
;; ? reaches the ROM through RDKEY; calls CMR; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
;; Leaves:    A, F, BC, DE, HL, IY
;;
;; ? calls GTNC, SETF7; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
CMD_COPY:
               ...
               CP &E7                          ; 59BE FE E7
               JR NZ,L59C8                     ; 59C0 20 06
               ; call &6C96 in the other page: LMPR is switched first, so that address is how the other listing numbers it
...
;; Leaves:    A, F, DE, HL
;;
;; ? reaches the ROM through OVERF; calls NRWR; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
L59E3:
               ...
               JR C,L59E3                      ; 59FC 38 E5  JR IF DIR TYPE
               CALL OHASR                      ; 59FE CD 2F 5D
               JR NZ,L59E3                     ; 5A01 20 E0  JR IF "?" AND "N"
               LD B,&0B                        ; 5A03 06 0B
...
;; Leaves:    A, F, BC, DE, HL
;;
;; ? calls PFNM0, SETF1, PMOG, FFPG; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
COYP3:
               ...
               INC B                           ; 5A31 04
               DEC B                           ; 5A32 05
               JP Z,L519B                      ; 5A33 CA 9B 51  ERROR IF NO FREE PAGES
...
;; Leaves:    A, F, BC, DE, HL
;;
;; ? calls DWAIT, HSVBK, PFNM0, SCFSM; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
BUDT:
               ...
               LD DE,MB_L7800                  ; 5ADD 11 00 B8  ALLOWS 1580H BYTES FOR SECTOR LIST
...
;; Leaves:    A, F, BC, DE, HL, IY
;;
;; ? reaches the ROM through CLSLOW; calls CMR; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
;; Leaves:    A, F, BC, DE, HL, IY
;;
;; ? calls CIEL, GTNC; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
;; Preserves: A, F (saved and restored)
;;
;; ? calls CIEL, ALLSR, SEPAR, EVEXP; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
;; Leaves:    A, F, BC, DE, HL, IY
;;
;; ? calls GCHR, EVFINS; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
;; Leaves:    A, F, BC, DE, HL, IY
;;
;; ? calls GTNC, ALLSR; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
;; Leaves:    A, F, BC, DE, HL, IY
;;
;; ? reaches the ROM through JCLSBL; calls CEOS, ALLSR, CMR; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
;; Leaves:    A, F, BC, DE, HL, IY
;;
;; ? calls SETBORDER_BORDCR, PDIRH, DITOB; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
L5A80:
               ...
               ; call &47FB in the other page: LMPR is switched first, so that address is how the other listing numbers it
...
;;
;; ? calls PFNM0; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
L5C8B:
               ...
               ; read the ROM variable WINDRHS -- the word below is its address, and the call returns past it
...
;; Leaves:    A, F, BC, DE, HL, IX
;;
;; ? calls SNDFX; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
ERAZ33:
               ...
               BIT 6,A                         ; 5CEC CB 77
               JR NZ,L5D1F                     ; 5CEE 20 2F  JR IF PROTECTED
...
;; Leaves:    A, F, BC, DE, HL, IX
;;
;; ? calls REFBUF, SNDFX; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
;; --------------------------------------------------------------------
;; CMD_PROTECT -- &5DE7 to &5DF0
;;
;; Takes:     BC, DE, HL
;; Leaves:    A, F, BC, DE, HL, IX, IY
;; Ends:      JR
;;
;; ? calls GTNC, BITF1, NMQU, SF1S.
;; --------------------------------------------------------------------
...
;; --------------------------------------------------------------------
;; CMD_HIDE -- &5DF1 to &5E0C
;;
;; Takes:     BC, DE, HL
;; Leaves:    A, F, BC, DE, HL, IY
;; Ends:      JR
;;
;; ? calls CEOS, GTNC, NRWRD, EVNUMX.
;; --------------------------------------------------------------------
...
;; Leaves:    A, F, BC, HL
;;
;; ? reaches the ROM through PROG; calls NRWRD; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------

; ---- L5E22 ---- from &5E0B, &5E19
L5E22:
               LD B,H                          ; 5E22 44
               LD C,L                          ; 5E23 4D
               ; write the ROM variable PROG
               CALL NRWRD                      ; 5E24 CD 69 50
               DEFW PROG                      ; 5E27 A0 5A
               IN A,(HMPR)                     ; 5E29 DB FB
               ; write the ROM variable PROGP
...
;; Leaves:    A, F, BC, DE, HL, IY
;;
;; ? calls NMQU, SF1S, EVFINS; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------

; ---- L5E31 ---- from &5DEF, &5DFB
L5E31:
...
;; Leaves:    A, F, BC, DE, HL, IX
;;
;; ? calls BITF1, SNDFX; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
SNDFX:
               ...
               JP NZ,L519E                     ; 5E5F C2 9E 51  "PROTECTED FILE"
...
; ---- REP26 ---- from &4EC3, &5F92, &5FA4, &6646, &6CC1, &6EEB
REP26:
...
;; Leaves:    F
;;
;; ? calls SEPAR; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
; ---- REP0 ---- from &5039, &67A2, &698E, &6A8D, &6B52, &78C0, &78C5
REP0:
...
EVPRM:
               ...
               LD A,H                          ; 5F22 7C
               CP &04                          ; 5F23 FE 04
               JP NC,L6091                     ; 5F25 D2 91 60  ALLOW 0000-03FFH IN DECED,
...
; ---- REMFP ---- from &5F66, &5F6F, &6A9A
REMFP:
               LD A,(HL)                       ; 5F55 7E
               CP &0E                          ; 5F56 FE 0E
               JR NZ,REMP1                     ; 5F58 20 08
               LD BC,&0006                     ; 5F5A 01 06 00
               ; call the ROM at JRECLAIM with ROM1 paged in, and page back on the way out
...
;; Leaves:    A, F, BC, HL
;;
;; ? reaches the ROM through PROG; calls NRRDD, BITF7; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------

; ---- DLVM1 ---- from &5FC0, &5FC5
DLVM1:
               CP &10                          ; 5FF7 FE 10
               ...
               PUSH BC                         ; 6006 C5
               POP HL                          ; 6007 E1
               ; read the ROM variable PROGP -- the word below is its address, and the call returns past it
               ...
               LD C,A                          ; 6014 4F
               PUSH BC                         ; 6015 C5
               ; read the ROM variable ELINE -- the word below is its address, and the call returns past it
               CALL NRRDD                      ; 6016 CD 53 50
               DEFW ELINE                     ; 6019 94 5A
               PUSH BC                         ; 601B C5
               POP HL                          ; 601C E1
               ; read the ROM variable ELINP -- the word below is its address, and the call returns past it
...
;; Leaves:    A, F, BC, DE, HL, IX
;;
;; ? calls NRWRD, RDBC, EVFL8B, TXHED; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------

; ---- DLVM2 ---- from &5FF9
DLVM2:
               LD IX,L4B00                     ; 604A DD 21 00 4B
...
; ---- L6091 ---- from &5969, &5E0E, &5F25, &60BB, &60CF, &60D8, &66C4, &6AAC ...
L6091:
...
;; Leaves:    A, F, BC, DE, HL, IY
;;
;; ? calls GCHR, EVNUMX; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
; ---- WIOOR ---- from &610D, &6122, &612A, &612E, &613D, &614A
WIOOR:
               JP L6091                        ; 60D8 C3 91 60
...
;; Ends:      JP, JR
;;
;; ? calls CALLMB, WAIT_DC_READY_BEFORE_CMD, RDDATA, CTAS.
;; --------------------------------------------------------------------
...
;; Leaves:    A, F, BC, DE, HL, IY
;;
;; ? calls CEOS, EVNAM2X; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
;; Leaves:    A, F, HL
;;
;; ? calls FTOSR, EVFINS; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
;; Ends:      JP, JR
;;
;; ? drives OUT (HMPR),A; calls CEOS, RTSTD, PTRD.
;; --------------------------------------------------------------------
...
;; Leaves:    A, F, BC, DE, HL, IY
;;
;; ? calls CEOS, EVFINS; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------

; ---- WFOD2 ---- from &60C6, &60CB, &60D6, &60DD
WFOD2:
...
;; Ends:      JR
;;
;; ? drives IN A,(HMPR), OUT (HMPR),A; calls NRWR, EVST2.
;; --------------------------------------------------------------------
...
;; Leaves:    A, F, BC, DE, HL
;;
;; ? drives OUT (HMPR),A; calls NRWR; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
;; Ends:      RET
;;
;; ? drives IN A,(HMPR), OUT (HMPR),A; calls EXDAT, EXDT1.
;; --------------------------------------------------------------------
...
;; Leaves:    A, F, BC, DE, HL
;;
;; ? calls BUDT, EXDAT; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
BSWOP:
               ...
               CALL BUDT                       ; 6272 CD D7 5A  GET HL=RPT, DE=PAGED IN BUFFER,
...
;; Leaves:    A, F, BC, DE, HL, IY
;;
;; ? reaches the ROM through GETSTR; calls CFSO, CMR; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
EVADDR:
               ...
               RET Z                           ; 62CB C8
               ; call the ROM at UNSTLEN with ROM1 paged in, and page back on the way out
...
;; Leaves:    A, F, DE, HL
;;
;; ? reaches the ROM through SLDEV; calls NRWR; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------

; ---- EVFL75 ---- from &634F
EVFL75:
               ; write the ROM variable SLDEV
               CALL NRWR                       ; 6356 CD 74 50
               DEFW SLDEV                     ; 6359 B7 5B
               LD A,(DSTR1)                    ; 635B 3A 36 41
               CP &08                          ; 635E FE 08
               JR NC,L6364                     ; 6360 30 02  JR IF NO SENSIBLE TAPE SPEED
...
;; Preserves: IX (saved and restored)
;;
;; ? calls CKDRV, GTFL3, RXHED2; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
;; Leaves:    A, F, BC, H
;;
;; ? drives IN A,(HMPR), OUT (C),B; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
;; Leaves:    A, F, BC, DE, HL, IX, IY
;;
;; ? calls CALLMB, NETPA; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------

HK_HLOAD:
               LD BC,SVBSI                     ; 6422 01 64 4A
               CALL NETPA                      ; 6425 CD 9D 64
               CALL L4F84                      ; 6428 CD 84 4F
...
L6443:
               ...
               POP AF                          ; 6451 F1
               ; call &62A6 in the other page: LMPR is switched first, so that address is how the other listing numbers it
...
L64F3:
               ...
               AND A                           ; 64FD A7
               JR Z,L654E                      ; 64FE 28 4E

; ---- L6500 ---- from MB &6173, MB &63A9
L6500:
               LD C,A                          ; 6500 4F
               LD A,(IX+&13)                   ; 6501 DD 7E 13
               CP &10                          ; 6504 FE 10
               JR Z,L654E                      ; 6506 28 46
               PUSH IX                         ; 6508 DD E5
               POP HL                          ; 650A E1
               ...
               LD C,A                          ; 6511 4F
               CP &14                          ; 6512 FE 14
               JR NZ,L651C                     ; 6514 20 06
               LD A,(V42BA)                    ; 6516 3A BA 42
               DEC A                           ; 6519 3D
               JR NZ,L6529                     ; 651A 20 0D
...
;; Leaves:    A, F, BC, HL
;;
;; ? calls CPPTR; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
; ---- L65C4 ---- from &79C7, &7B64
L65C4:
               LD B,A                          ; 65C4 47
               OR H                            ; 65C5 B4
               OR L                            ; 65C6 B5
               JR Z,L65CE                      ; 65C7 28 05  ZERO IS A SPECIAL CASE
               LD A,B                          ; 65C9 78
...
; ---- L65CE ---- from &65C7, &79A7, &79E2, &7B4B
L65CE:
               LD A,H                          ; 65CE 7C
               LD H,L                          ; 65CF 65
               LD L,A                          ; 65D0 6F
...
; ---- AUINC ---- from &661E
AUINC:
               CALL L4E75                      ; 65F7 CD 75 4E
               JP L5FBB                        ; 65FA C3 BB 5F
...
AUINSR:
               ...
               DEFB &21                                                         ; 6604 !  skipped: reads as LD HL,AUTNAM from here, and as part of the instruction above it
               PUSH DE                         ; 6605 D5
               LD H,L                          ; 6606 65
...
INIT:
               DEFB &CD,&FD                                                     ; 661A M}  skipped: reads as CALL AUINSR from here, and as part of the instruction above it
               LD H,L                          ; 661C 65
...
;; Ends:      JP, JR
;;
;; ? drives IN A,(HMPR), OUT (HMPR),A; calls HFWCD, ITRCK, ISECT, CHKHL.
;; --------------------------------------------------------------------
...
;; Leaves:    A, F, BC, IY
;;
;; ? calls DWAIT, HFWCD, SELD; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
;; Leaves:    A, F, BC, DE, HL, IX
;;
;; ? drives IN A,(HMPR), OUT (HMPR),A; calls HFWCD; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
;; Preserves: HL (saved and restored)
;;
;; ? calls ITRCK, ISECT, CHKHL; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
FRW2:
               ...
               JP L4F84                        ; 66B2 C3 84 4F  NORMAL BUF AGAIN
...
SCASD:
               ...
               INC A                           ; 66C3 3C
               JP Z,L6091                      ; 66C4 CA 91 60  0-3FFFH ILLEGAL ADDR
...
;; Leaves:    A, F, HL
;;
;; ? reaches the ROM through DEVL; calls NRRD; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------

; ---- GTDEF ---- from &660F, &6701, &7AB0, &7B98
GTDEF:
               ; read the ROM variable DEVL -- the word below is its address, and the call returns past it
...
; ---- GTDD ---- from &6183
GTDD:
               ; read the ROM variable DEVN -- the word below is its address, and the call returns past it
               CALL NRRD                       ; 66DF CD 5E 50
               DEFW DEVN                      ; 66E2 07 5A
               AND A                           ; 66E4 A7
               JR Z,L66EB                      ; 66E5 28 04
               CP RDLIM+1                      ; 66E7 FE 09
               JR C,L66EE                      ; 66E9 38 03  JR IF LEGAL DRIVE
...
;; Leaves:    F, B
;;
;; ? calls C11SP; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
EVFL11:
               ...
               JR NZ,L6766                     ; 673D 20 27  JR IF NOT "D1      "
...
; ---- EVFL12 ---- from &6729, &6738
EVFL12:
               SUB &30                         ; 6747 D6 30
               CP &0A                          ; 6749 FE 0A
               JR NC,L6766                     ; 674B 30 19  JR IF 2ND DIGIT NOT FOUND
               LD E,A                          ; 674D 5F
               LD A,D                          ; 674E 7A
               ADD A,A                         ; 674F 87
               ADD A,A                         ; 6750 87
...
;; Leaves:    A, F
;;
;; ? calls CODN; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
; ---- L6766 ---- from &6721, &673D, &674B, &6759
L6766:
...
;; Leaves:    A, F, BC, DE, HL
;;
;; ? calls LCNTA; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
;; Leaves:    A, F, BC, D, HL
;;
;; ? calls EXDAT, MOVJ, TOSCQ; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
MOVA:
               ...
               POP AF                          ; 683B F1
               CP &10                          ; 683C FE 10
               JR NZ,L687C                     ; 683E 20 3C  IF NOT PROGRAM, SUPPRESS
               XOR A                           ; 6840 AF
...
;; Leaves:    A, F, B, HL
;;
;; ? calls PNUM5, MOVJ, MOVRC; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
;; Leaves:    A, F, BC, IX
;;
;; ? calls EXDAT, CLMOV; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------

; ---- MEOF ---- from &6849, &687F, &6894
MEOF:
               XOR A                           ; 689B AF
...
;; Leaves:    A, F, BC, HL
;;
;; ? calls MCHRD; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
BKU2:
               ...
               LD BC,&0028                     ; 69FB 01 28 00
               ADD HL,BC                       ; 69FE 09  ALLOW FOR DIRECTORY TKS 0-3
               INC B                           ; 69FF 04  NZ
               LD DE,&0001                     ; 6A00 11 01 00
...
;; Ends:      JR, RET
;;
;; ? drives OUT (HMPR),A; calls SETF6, BITF1, BITF6, FESE2.
;; --------------------------------------------------------------------
...
BKUL:
               ...
               LD A,B                          ; 6A0F 78
               AND A                           ; 6A10 A7
               JP Z,L519B                      ; 6A11 CA 9B 51  ERROR IF NO PAGES FREE
...
;; Leaves:    F, HL
;;
;; ? calls SETF1; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
;; Leaves:    A, F, BC, DE, HL, IX, IY
;;
;; ? drives OUT (HMPR),A; calls SETF6, BITF6, FESE2, TSPCE1; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
;; --------------------------------------------------------------------
;; CMD_OPEN -- &6A97 to &6ABE
;;
;; Takes:     A, DE, HL
;; Leaves:    A, F, BC, DE, HL, IY
;;
;; ? calls CFSO, CEOS, GTNC, REMFP; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
CMD_OPEN:
               ...
               CP &A9                          ; 6AA0 FE A9
               JR NZ,L6B21                     ; 6AA2 20 7D
               CALL EVNUMX                     ; 6AA4 CD AF 62
               CALL CEOS                       ; 6AA7 CD 07 50
               INC B                           ; 6AAA 04
               DEC B                           ; 6AAB 05
               JP NZ,L6091                     ; 6AAC C2 91 60
               LD A,C                          ; 6AAF 79
               LD B,A                          ; 6AB0 47
               DEC A                           ; 6AB1 3D
               CP &06                          ; 6AB2 FE 06
               JP NC,L6091                     ; 6AB4 D2 91 60
...
;; Leaves:    A, F, BC, DE, HL, IY
;;
;; ? calls CEOS, PLNS, SEPARX, EVSRM; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
L6B84:
               ...
               CP &0D                          ; 6B89 FE 0D
               JR Z,L6BD5                      ; 6B8B 28 48  JR IF CHANS TERMINATOR FOUND
               LD A,(IX+&04)                   ; 6B8D DD 7E 04
               AND A                           ; 6B90 A7
               JR NZ,L6B97                     ; 6B91 20 04
...
; ---- L6BCD ---- from &6B9B, &6BA3, &6BAA, &6BB1, &6BC4
L6BCD:
...
OPND45:
               ...
               CP MRND                         ; 6CCB FE A5
               LD A,&02                        ; 6CCD 3E 02
               JR Z,L6CD2                      ; 6CCF 28 01  JR IF RND (BITS 1-0 = 10)
...
;; Leaves:    A, F, BC, DE, HL, IX, IY
;;
;; ? reaches the ROM through JRECLAIM; calls ROFSM, DDEL, RAMST, CMR; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
L6D5B:
               ...
               LD A,B                          ; 6D5F 78
               OR C                            ; 6D60 B1
               JR NZ,L6D5B                     ; 6D61 20 F8
               JP L6AE4                        ; 6D63 C3 E4 6A
...
;; Leaves:    A, F, BC, DE, HL
;;
;; ? calls BITF1, SDCM; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
SDCM:
               ...
               BIT 5,(IX+&0C)                  ; 6E94 DD CB 0C 6E
               RET Z                           ; 6E98 C8  RET IF FILE WAS NOT ALTERED - NO
               CALL GLEN                       ; 6E99 CD D4 71  GET FILE LEN IN AHL, SECTS IN BC
               LD D,A                          ; 6E9C 57
               LD A,(IX+&13)                   ; 6E9D DD 7E 13
               AND &1F                         ; 6EA0 E6 1F
               CP &10                          ; 6EA2 FE 10
               LD A,D                          ; 6EA4 7A
               JR C,SDCM2                      ; 6EA5 38 06  JR IF OPENTYPE AND ZX FILES
               LD DE,&FFF7                     ; 6EA7 11 F7 FF
               ADD HL,DE                       ; 6EAA 19
...
SDCM2:
               ...
               DEC HL                          ; 6EBB 2B
               DEC HL                          ; 6EBC 2B
               LD (HL),A                       ; 6EBD 77  HI - BYTE D2
               EX DE,HL                        ; 6EBE EB
               CALL PAGEFORM                   ; 6EBF CD E4 75
               EX DE,HL                        ; 6EC2 EB
               LD BC,&001D                     ; 6EC3 01 1D 00
               ...
               INC HL                          ; 6ECC 23
               LD (HL),&FF                     ; 6ECD 36 FF  "EXEC ADDR" OF FFXXXX
               PUSH IX                         ; 6ECF DD E5
               BIT 2,(IX+&0C)                  ; 6ED1 DD CB 0C 56
               JP Z,CLOSX                      ; 6ED5 CA 13 4E  JR IF FILE IS A NEW ONE - MAKE
               PUSH IX                         ; 6ED8 DD E5
               POP HL                          ; 6EDA E1
               ...
               LD A,&08                        ; 6EE6 3E 08
               CALL FDHR                       ; 6EE8 CD 31 4B  LOOK FOR FILE ENTRY
               JP NZ,REP26                     ; 6EEB C2 62 5E  ERROR IF NOT FOUND
               JP NCF25                        ; 6EEE C3 1B 4E  UPDATE DIRECTORY - OVERWRITING
...
;; Leaves:    A, F
;;
;; ? calls LBYT; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
MCHWR:
               ...
               JR Z,L6F35                      ; 6F58 28 DB  NO WRITE IF SO
...
HK_SBYT:
               ...
               LD (HL),E                       ; 6F83 73
               EX DE,HL                        ; 6F84 EB
               CALL L4FCD                      ; 6F85 CD CD 4F  PLACE NEXT T/S IN IX+,
...
L6F8D:
               ...
               POP HL                          ; 6F9A E1
               JR NZ,L6FB6                     ; 6F9B 20 19  JR IF WE ARE NOT AT FILE END
...
;; Preserves: DE (saved and restored)
;;
;; ? calls WRIF2; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
CPPTR:
               ...
               RET NZ                          ; 6FE9 C0
               INC HL                          ; 6FEA 23
               LD A,(IX+&1F)                   ; 6FEB DD 7E 1F
               CP (HL)                         ; 6FEE BE
               RET NZ                          ; 6FEF C0
               INC HL                          ; 6FF0 23
               LD A,(IX+&1E)                   ; 6FF1 DD 7E 1E
               SUB (HL)                        ; 6FF4 96
...
RAMST:
               PUSH HL                         ; 705D E5
               LD (IX+&0C),A                   ; 705E DD 77 0C
               PUSH IX                         ; 7061 DD E5
               POP HL                          ; 7063 E1
               LD BC,WRRAM                     ; 7064 01 13 01
               ADD HL,BC                       ; 7067 09
               LD (IX+&0F),L                   ; 7068 DD 75 0F
               LD (IX+&10),H                   ; 706B DD 74 10
               LD A,(DSTR1)                    ; 706E 3A 36 41
               LD (IX+&0B),A                   ; 7071 DD 77 0B
               POP HL                          ; 7074 E1
               RET                             ; 7075 C9
...
;; --------------------------------------------------------------------
;; Fix_L6DD1_43 -- &709A to &70AD
;;
;; Takes:     A, C, HL, IX
;; Leaves:    A, F, BC, DE, HL
;;
;; ? calls D510; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
Fix_L6DD1_43:
               ...
               ; to the alternate register set and back again
               EXX                             ; 70A6 D9
               LD (IX+&1F),L                   ; 70A7 DD 75 1F
               LD (IX+&1E),H                   ; 70AA DD 74 1E
               ; to the alternate register set and back again
               EXX                             ; 70AD D9
...
;; Preserves: DE (saved and restored)
;;
;; ? calls WRIF, FNS5; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
FTSSL:
               ...
               LD H,D                          ; 70F1 62
               INC L                           ; 70F2 2C
               CALL L4FBF                      ; 70F3 CD BF 4F
               EX DE,HL                        ; 70F6 EB
               AND A                           ; 70F7 A7
               SBC HL,DE                       ; 70F8 ED 52
               JR Z,L7104                      ; 70FA 28 08
               PUSH DE                         ; 70FC D5
...
ICNT:
               INC (IX+&1F)                    ; 71A9 DD 34 1F
               RET NZ                          ; 71AC C0
               INC (IX+&1E)                    ; 71AD DD 34 1E
               RET                             ; 71B0 C9
...
;; Leaves:    A, F, BC, DE, HL, IY
;;
;; ? drives IN A,(HMPR); calls CEOS, GTNC; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
;; Leaves:    A, F, BC, DE, HL, IY
;;
;; ? calls CEOS, EVNAM; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
;; Leaves:    A, HL
;;
;; ? calls GCDIA, GPLA; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
;; Ends:      RET
;;
;; ? drives IN A,(HMPR), OUT (HMPR),A; calls PRPTH, DTREE.
;; --------------------------------------------------------------------
...
; ---- PRPD ---- from &734B
PRPD:
               DEFB &20,&2A                                                     ; 7358 *  skipped: reads as JR NZ,&7384 from here, and as part of the instruction above it
...
;; Leaves:    A, F, BC, HL
;;
;; ? calls GPATD; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
;; Leaves:    A, F, C, DE, HL
;;
;; ? calls GPLA; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
SDTKS:
               ...
               LD A,C                          ; 746A 79
               CP (HL)                         ; 746B BE
               JR NZ,L7472                     ; 746C 20 04  JR IF NEW DISC
               INC HL                          ; 746E 23
               LD A,B                          ; 746F 78
...
;; Preserves: A (saved and restored)
;;
;; ? calls SETRT; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
L7488:
               ...
               ; call &5044 in the other page: LMPR is switched first, so that address is how the other listing numbers it
...
; ---- RDW4 ---- from &753F, &7559, &7561
RDW4:
               POP DE                          ; 74E3 D1  T/S
               CALL L4F8E                      ; 74E4 CD 8E 4F
...
NRDROL:
               ...
               INC D                           ; 7522 14
               AND A                           ; 7523 A7
               JR Z,L752E                      ; 7524 28 08  JR IF AN ERASED ENTRY
               DEC D                           ; 7526 15
...
;; Ends:      JR
;;
;; ? drives OUT (HMPR),A; calls CPFTS, PTRD2.
;; --------------------------------------------------------------------
...
;; Leaves:    A, HL
;;
;; ? calls GFPA; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
;; Leaves:    A, F, BC, DE, HL
;;
;; ? calls WSAD, CCNT, FFNS, CHKHL; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
RDSBL:
               ...
               EX DE,HL                        ; 761C EB
               CALL L4FCD                      ; 761D CD CD 4F  GET T/S IN D/E
...
;; Leaves:    A, F, BC, DE, HL
;;
;; ? calls CPFTS, CNTFP; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
;; Leaves:    F, BC, DE, HL
;;
;; ? calls SELRDP; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
;; Preserves: A, F (saved and restored)
;;
;; ? calls FESET, GFPA; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
;; Leaves:    A, F, BC
;;
;; ? calls MRADDR; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
CFPBL:
               ...
               JR Z,L7855                      ; 784D 28 06  JR IF HIT 256 WHEN CALL FROM BOOT
...
HKLEN:
               ; to the alternate register set and back again
               EXX                             ; 7893 D9  HL=RET ADDR TO SCANNING
               INC HL                          ; 7894 23
               INC HL                          ; 7895 23
               INC HL                          ; 7896 23
               LD C,(HL)                       ; 7897 4E  DISP TO "IMMED CODES"
...
; ---- L78E5 ---- from &78BC
L78E5:
               ; call &5E64 in the other page: LMPR is switched first, so that address is how the other listing numbers it
...
;; Leaves:    A, F, BC, DE, HL, IY
;;
;; ? calls ALLSR, EVNAMX, EVFINS; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
;; Leaves:    A, F, BC, DE, HL, IY
;;
;; ? calls ISEP, GCHR, FABORT; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
; ---- HVAR2H ---- from &7971
HVAR2H:
               JP L658D                        ; 7965 C3 8D 65  STACK-1
...
;; Ends:      JP, JR
;;
;; ? drives IN A,(HMPR), OUT (HMPR),A; calls CKDRV, ZDVS, DITOB, STATS.
;; --------------------------------------------------------------------
...
;; Leaves:    A, F, BC, DE, HL
;;
;; ? drives IN A,(HMPR), OUT (HMPR),A; calls ZDVS, DITOB, STATS; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
; ---- SHLHP ---- from &79AE
SHLHP:
               JP Z,L65CE                      ; 79A7 CA CE 65  NO SPACE IF NO SLOTS
               JR DST18                        ; 79AA 18 07
...
;; Leaves:    A, F, BC
;;
;; ? calls WPCHK; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
; ---- STKA ---- from &7955, &797C, &7989, &79B5, &79DA, &79EE, &7B43
STKA:
               LD L,A                          ; 79DF 6F
               LD H,&00                        ; 79E0 26 00
...
; ---- DSSTK ---- from &79CF, &79D5
DSSTK:
               JP L65CE                        ; 79E2 C3 CE 65
...
INPST:
               ...
               LD HL,&0010                     ; 7A19 21 10 00
               AND A                           ; 7A1C A7
               SBC HL,DE                       ; 7A1D ED 52
               JP C,INVST                      ; 7A1F DA 44 70  LIMIT STREAM TO 0-16
               PUSH BC                         ; 7A22 C5
               ; read the ROM variable CURCHL -- the word below is its address, and the call returns past it
               ...
               LD A,B                          ; 7A35 78
               OR C                            ; 7A36 B1
               JR Z,L7A43                      ; 7A37 28 0A
               PUSH BC                         ; 7A39 C5
               DEC BC                          ; 7A3A 0B  Z->FFFF
               LD A,B                          ; 7A3B 78
               CP &40                          ; 7A3C FE 40
               JP NC,L6091                     ; 7A3E D2 91 60
               JR L7A46                        ; 7A41 18 03
...
;; Leaves:    A, F, BC, DE, HL, IY
;;
;; ? reaches the ROM through WKROOM; calls CMR; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
INPSL:
               ...
               INC DE                          ; 7A65 13
               INC B                           ; 7A66 04
               JR NZ,L7A7D                     ; 7A67 20 14
               CP &0D                          ; 7A69 FE 0D
               JR NZ,L7A76                     ; 7A6B 20 09
               EX DE,HL                        ; 7A6D EB
               POP DE                          ; 7A6E D1
               POP BC                          ; 7A6F C1
               SCF                             ; 7A70 37
...
;; Leaves:    A, F, BC, DE, HL, IY
;;
;; ? calls ISEP, EVNUM; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
FSTAT:
               ...
               JP C,L6091                      ; 7AC4 DA 91 60
               CALL EVFINS                     ; 7AC7 CD 21 73
               CALL HOCHK                      ; 7ACA CD 67 7B
               POP BC                          ; 7ACD C1
               JP Z,L658D                      ; 7ACE CA 8D 65  STACK -1 IF NO HOLE OR
               PUSH BC                         ; 7AD1 C5
               CALL FINDC                      ; 7AD2 CD A7 4F  LOOK FOR NAMED FILE, POINT
               POP BC                          ; 7AD5 C1
               JR NZ,L7B10                     ; 7AD6 20 38  JR IF NOT FOUND - STACK 0
               DEC C                           ; 7AD8 0D
               JR Z,L7B46                      ; 7AD9 28 6B
               LD D,&00                        ; 7ADB 16 00
               LD A,(HL)                       ; 7ADD 7E  TYPE
               DEC C                           ; 7ADE 0D
               JR Z,L7B4E                      ; 7ADF 28 6D
               DEC C                           ; 7AE1 0D
               JR Z,L7B41                      ; 7AE2 28 5D
               DEC C                           ; 7AE4 0D
               JR Z,L7B43                      ; 7AE5 28 5C
               DEC C                           ; 7AE7 0D
               JR NZ,L7AF3                     ; 7AE8 20 09  JR IF PARAM WAS 4
               LD E,&EC                        ; 7AEA 1E EC
               ADD HL,DE                       ; 7AEC 19
               LD A,(HL)                       ; 7AED 7E
               AND &1F                         ; 7AEE E6 1F
...
L7AF3:
               ...
               LD A,(HL)                       ; 7AFC 7E
               INC A                           ; 7AFD 3C
               JR Z,L7B43                      ; 7AFE 28 43
               LD A,B                          ; 7B00 78
               CP &13                          ; 7B01 FE 13
               JR Z,L7B13                      ; 7B03 28 0E
               CP &10                          ; 7B05 FE 10
               JR NZ,L7B10                     ; 7B07 20 07
               INC HL                          ; 7B09 23
               LD E,(HL)                       ; 7B0A 5E
               INC HL                          ; 7B0B 23
               LD D,(HL)                       ; 7B0C 56
...
; ---- L7B43 ---- from &7AE5, &7AFE, &7B11, &7B21, &7B28
L7B43:
...
; ---- L7B46 ---- from &7AD9
L7B46:
               CALL L4FD7                      ; 7B46 CD D7 4F  CONVERT T/S IN D/E TO A NUMBER IN BC
               LD H,B                          ; 7B49 60
               LD L,C                          ; 7B4A 69
...
;; Leaves:    A, F, H
;;
;; ? calls AHLNX; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
;; Leaves:    A, F, BC, D
;;
;; ? calls TFIHO; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
;; Leaves:    A, F, BC, HL
;;
;; ? calls GTDEF, GPATD; falls into whatever follows rather than returning.
;; --------------------------------------------------------------------
...
;; --------------------------------------------------------------------
               ...
               LD HL,L58A4                     ; 7BED 21 A4 58  RET ADDR TO PAGING SR
...
FSA:
               DEFB &80,&CD,&3D,&BD,&CD,&2C,&BD,&F5,&3E,&01,&01,&D9,&67,&21,&00 ; 7C13 .M==M,=u>..Yg!.  the 256-byte image of a directory entry
...
; ---- MBCOPY_7774 ---- from MB &7767
MBCOPY_7774:
               DEFB &5C,&01,&05,&00,&ED,&B0,&ED,&53,&65,&5C,&CD,&7E,&01,&EB,&78 ; 7D93 \...m0mSe\M~.kx
               DEFB &B1,&C8,&DB,&FB,&F5,&E5,&C5,&3A                             ; 7DA2 1H[{ueE:

; ---- MBCOPY_778B ---- from MB &76E1
MBCOPY_778B:
...
MBCOPY_7829:
               DEFB &3E,&22,&22,&00,&08,&14,&00,&1C,&22,&3E,&22,&00,&0C,&10,&3E ; 7E48 >"".....">"...>
               DEFB &20,&3C,&20,&3E,&00,&00,&00,&04,&1A,&2E,&38,&2E,&00,&1E,&28 ; 7E57  < >......8...(
               DEFB &28,&3C,&28,&28,&2E,&00,&08,&14,&00,&1C,&22,&22,&1C,&00,&00 ; 7E66 (<((......""...
               DEFB &14,&00,&1C,&22,&22,&1C,&00,&18,&04,&00,&1C,&22,&22,&1C,&00 ; 7E75 ...""......""..
...
V7EA6:
               ...
               DEFB &3C,&22,&3C,&20,&28,&2C,&28,&06,&04,&0A,&08,&1C,&08,&28,&10 ; 7EC4 <"< (,(......(.
               DEFB &00,&0C,&10,&00,&1A,&26,&26,&1A,&00,&0C,&10,&00,&08,&08,&08 ; 7ED3 .....&&........
...
STR:
               DEFB &18,&23,&E1,&FE,&06,&20,&D1,&21,&6A,&5C,&7E,&EE,&08,&77,&BF ; 7F90 .#a~. Q!j\~n.w?  REGISTER STORE AND STACK
